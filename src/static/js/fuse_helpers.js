...
async function getFuseUniPool(App, pool, poolAddress, stakingAddress) {
    let q0, q1;
...
async function getFuseVault(App, vault, address, stakingAddress) {
...
async function getFuseWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getFuseStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
...
async function loadFuseSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function loadFuseSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
...
async function loadFuseBasisFork(data) {
...
async function getFusePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadFuseChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadMultipleFuseSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
